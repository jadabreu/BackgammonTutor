...
public class GameOptionsTable extends Table {
  ...
  public GameOptionsTable(boolean decoration, ClickListener _cl) {
    ...
    TextButton mg2 = new TextButton("No", ts);
    mgreedy.add(mg1);
    mgreedy.add(mg2);
    ...
    ClickListener cl2 = new ClickListener() {
      public void clicked(InputEvent event, float x, float y) {
        ...
        String s = ((TextButton)event.getListenerActor()).getText().toString().toUpperCase();
        if (s.equals("TAP"))
          setDisabledLmoves(false);
        if (s.equals("AUTO"))
          setDisabledLmoves(true);
      };
    };
    ...
    row().spaceBottom(height * 0.1f);
    add().fill().height(height * 1.05f).expandX();
    add(new Label("Greedy Bearoff:", GnuBackgammon.skin)).right().spaceRight(6);
    add(mg1).height(height * 1.3f).width(width);
    add(mg2).height(height * 1.3f).width(width);
    add().fill().expandX();
  ...
  public void initFromPrefs() {
    ...
    if (automoves.equals("Tap"))
      setDisabledLmoves(false);
    if (automoves.equals("Auto"))
      setDisabledLmoves(true);
